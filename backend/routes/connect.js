...
router.get('/oauth/:provider', protect, authorize('teamadmin', 'superadmin'), (req, res) => {
  const { provider } = req.params;
  const authUrl = providers.getOAuthUrl(provider, req.user.companyId);
...
router.post('/sync/:provider', protect, authorize('teamadmin', 'superadmin'), async (req, res) => {
  ...
  try {
    ...
    const { accessToken } = conn.docs[0].data();
