...
async function validateApiKey(provider, apiKey) {
  try {
    if (provider === 'openai') {
      await httpGet('https://api.openai.com/v1/organizations', {
        Authorization: `Bearer ${apiKey}`
      });
      return true;
    }
    if (provider === 'anthropic') {
      await httpGet('https://api.anthropic.com/v1/organizations', {
        'x-api-key': apiKey,
...
async function syncAnthropic(apiKey) {
  const data = await httpGet('https://api.anthropic.com/v1/usage', {
    'x-api-key': apiKey,
    'anthropic-version': '2023-06-01'
  });
  const events = [];
  for (const item of data?.data || []) {
    events.push({
      ...
      costUsd: ((item.input_tokens || 0) * 0.000003) + ((item.output_tokens || 0) * 0.000015),
      eventType: 'completion',
...
async function syncAllConnections() {
  ...
  try {
    ...
    for (const doc of connections.docs) {
      const { companyId, provider, accessToken } = doc.data();
